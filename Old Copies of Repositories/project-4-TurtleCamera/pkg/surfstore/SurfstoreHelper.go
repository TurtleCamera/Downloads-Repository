...
func WriteMetaFile(fileMetas map[string]*FileMetaData, baseDir string) error {
	...
	for _, fileMeta := range fileMetas {
		...
		for index, hashValue := range valuesList {
			...
			_, err := insertStatement.Exec(name, version, index, hashValue)
			if err != nil {
