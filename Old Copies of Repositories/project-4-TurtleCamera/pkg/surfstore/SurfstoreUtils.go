...
func downloadFile(client RPCClient, remoteMetaData *FileMetaData) error {
    ...
    // Ashrya told me that we're not supposed to have access to the functions
	// in ConsistentHashRing in a real-life setting. Instead, he suggests to
	// call GetBlockStoreMap on hashList and "invert" it (extract all hashes
	// from the value in the key-value pair and map each hash to the server
	// address its list was associated with in the pair).
	...
	if err != nil {
		return fmt.Errorf("Error getting block store map: %v", err)
	}

	// "Invert" the map by extracting the hashes out the lists and mapping
	// them to their respective server addresses
	hashToServerMap := make(map[string]string)
	for blockAddress, hashes := range blockStoreMap {
		for _, hash := range hashes {
			hashToServerMap[hash] = blockAddress
		}
	}

	// If we're here, then write all the blocks to the client
	for _, hash := range hashList {
        // Determine which block store is responsible for this block
        blockAddress := hashToServerMap[hash]

		// Get the block
		var block Block
		if hash == "-1" {
			// Ashrya's comment: For an empty file, the hash entry of -1 (as stated in FAQ
			//                   of project 3) will only be added to local index.db and the
			//                   metastore. You should not be adding it to the blockstore.
			// I'm not sure if he means to skip calling PutBlock when we have an empty block.
			block = Block{
				BlockData: []byte{},
				BlockSize: 0,
			}
...
func uploadFile(client RPCClient, metaData *FileMetaData) error {
	baseDirectory := client.BaseDir
	...
	// Create a map that maps the block hash values to the blocks themselves
    fileStat, _ := file.Stat()
	totalFileSize := float64(fileStat.Size())
	blockSize := float64(client.BlockSize)
    numBlocks := int(math.Ceil(totalFileSize / blockSize))
	blockHashMap := make(map[string]*Block, numBlocks)
	
	// The hashs depends on whether or not the file is empty
	if totalFileSize <= 0 {
		// Change the map to store one block because numBlocks would
		// have been 0 in this case
		blockHashMap = make(map[string]*Block, 1)
		...
		// Map the hash to the block
		blockHashMap[fileHash] = block
	} else {
		count := 0
		for count < numBlocks {
			// Read in the slice
			...
			// Trim the slice and calculate the hash
			fileHash := GetBlockHashString(slice[:n])
	...
	// Extract all the keys (block hashes) from blockHashMap
	blockHashes := make([]string, 0, len(blockHashMap))
	for hash := range blockHashMap {
		blockHashes = append(blockHashes, hash)
	}

    // Get the block store map from the MetaStore server
    blockStoreMap := make(map[string][]string)
	...
	// Loop through all address-hash list pairs and put blocks in their respective servers
    for blockAddress, hashList := range blockStoreMap {
		// Loop through the hashest in the hashList and use the block store address
		// of the current outer loop iteration to put blocks in that server
		for _, hash := range hashList {
			// Ashrya's comment: For an empty file, the hash entry of -1 (as stated in FAQ
			//                   of project 3) will only be added to local index.db and the
			//                   metastore. You should not be adding it to the blockstore.
			...
			// Retrieve the block from the blockHashMap map made earlier
			block := blockHashMap[hash]
			var succ bool
...
func handleDownload(client RPCClient, metaIndices, remoteIndices map[string]*FileMetaData) {
	for name, remoteMetaData := range remoteIndices {
		...
		if shouldDownload(exists, localMetaData, remoteMetaData) {
			...
			err := downloadFile(client, remoteMetaData)
			if err != nil {
...
func handleUpload(client RPCClient, metaIndices, remoteIndices map[string]*FileMetaData) {
	for name, localMetaData := range metaIndices {
		...
		if shouldUpload(exists, localMetaData, remoteMetaData) {
			err := uploadFile(client, localMetaData)
			if err != nil {
...
func uploadAndDownload(client RPCClient, metaIndices, remoteIndices map[string]*FileMetaData) {
	// Handle downloading of files from server to client
	handleDownload(client, metaIndices, remoteIndices)
	
	// Handle uploading of files from client to server
	handleUpload(client, metaIndices, remoteIndices)
}
...
func ClientSync(client RPCClient) {
	...
	uploadAndDownload(client, metaIndices, remoteIndices)
